...
class GaussianQRC:
    def __init__(self, n, encoding_strength, coupling_strength, gamma, dt, cov_measurements="full", encoding_mode="one_to_one", return_fourth_moments=False, T=0.0, seed=42, n_shots=shots.EXACT):
        """
        ...
        - n_shots: Number of measurement shots per readout setting. float('inf') (the
                    default) means exact expectation values. The homodyne readout is a
                    single setting, so this is also the total per time step. Only the
                    q-only family of cov_measurements is supported for finite shots.
        """
        self.n = n
        ...
        # One homodyne setting reads the q quadrature of every mode at once, so the total
        # measurement cost per time step is n_shots, with no per-setting multiplier.
        self.n_settings = 1
        ...
        # Only the q-only family is a single measurement setting. 'full', 'diag', 'p_only'
        # and 'full_3' contain p-quadrature or mixed qp entries, and q_i and p_i do not
        # commute, so no single homodyne setting produces them and one Wishart draw is not
        # their sampling distribution. Refuse rather than sample something unphysical.
        self._finite_shots = not shots.is_exact(self.n_shots)
        q_only_modes = ('q_only', 'q_diag', 'q_only_3')
        if self._finite_shots and self.cov_measurements not in q_only_modes:
            raise NotImplementedError(
                f"finite shots are not modelled for cov_measurements={self.cov_measurements!r}: "
                f"it contains p-quadrature or mixed qp entries, which no single homodyne "
                f"setting can measure. Supported: {q_only_modes}"
            )
        if self._finite_shots:
            shots.validate_n_shots(self.n_shots, minimum=self.n, what="the number of modes")
        ...
        # Only on the finite-shot path, so the exact path is provably free of measurement
        # randomness.
        self._shot_rng = shots.shot_rng(seed) if self._finite_shots else None
    ...
    def run(self, input_sequence, return_negativity=False, return_purity=False, return_squeezing=False) -> tuple:
        ...
        for t in range(seq_len):
            ...
            # The restarting protocol: the readout is estimated from a finite ensemble, but
            # `current_cov` -- the state the next step evolves from -- is untouched, so the
            # trajectory is the noiseless one and the readout noise is independent across
            # steps. The sampled q-block is embedded back into a full 2n x 2n array so the
            # precomputed flattening indices need no special case; the nan fill means a mode
            # that reads a p entry yields nan rather than a plausible-looking number, behind
            # the constructor's refusal to accept such a mode at all.
            if self._finite_shots:
                readout_cov = np.full_like(current_cov, np.nan)
                readout_cov[::2, ::2] = shots.sample_q_covariance(
                    current_cov[::2, ::2], self.n_shots, self._shot_rng
                )
            else:
                readout_cov = current_cov
            flat_cov = self._flatten_covariance(readout_cov)

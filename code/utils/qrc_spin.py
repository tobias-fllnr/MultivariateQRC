...
class SpinQRC:
    def __init__(self, n: int, encoding_strength: float, coupling_strength: float, gamma: float, dt: float, model: str = "TiltedTFIM", encoding_method: str = "one_to_one", observables: str = "local_and_twoqubit", seed: int = 42, n_shots: float = shots.EXACT):
        ...
        Args:
            ...
            n_shots (float, optional): Measurement shots per readout setting.
                float('inf') (the default) means exact expectation values. The local
                Pauli readout needs three non-commuting settings (X, Y, Z), so the total
                cost per time step is 3 * n_shots. Only the "local" and
                "local_and_twoqubit" observable sets are supported for finite shots.
        """
        self.n = n
        ...
        # _get_observables() reads self.observables as a string and then replaces it with
        # the operator list, so the spec is captured here for the shot path, which needs to
        # know which readout layout it is sampling.
        self.observables_spec = observables
        self.n_shots = n_shots
        self._finite_shots = not shots.is_exact(n_shots)
        # Three non-commuting settings: all-X, all-Y, all-Z. Within a setting the
        # single-qubit Paulis on distinct qubits commute, so one shot estimates all n of
        # them at once. This is the factor 3 in Mujal et al. Eqs. (G1)-(G3).
        self.n_settings = 3
        self.seed = seed

        # These two sets are the ones whose first 3n columns are the local (x, y, z) block
        # the four DV task functions read. The others have a different layout or would need
        # more than three settings, and no task function uses them.
        local_specs = ("local", "local_and_twoqubit")
        if self._finite_shots and self.observables_spec not in local_specs:
            raise NotImplementedError(
                f"finite shots are not modelled for observables={self.observables_spec!r}: "
                f"only the local Pauli readout's three commuting settings are budgeted. "
                f"Supported: {local_specs}"
            )
        ...
        if self._finite_shots:
            shots.validate_n_shots(n_shots, minimum=1, what="one shot per setting")
            self._basis_rotations = shots.pauli_basis_rotations(self.n)
            self._outcome_signs = shots.outcome_signs(self.n)
            self._shot_rng = shots.shot_rng(seed)
        else:
            self._basis_rotations = None
            self._outcome_signs = None
            self._shot_rng = None
    ...
    def run(self, data: np.ndarray, return_negativity: bool = False, return_coherence: bool = False) -> np.ndarray:
        ...
        for i, dat in enumerate(data):
            ...
            if self._finite_shots:
                # The restarting protocol: rho keeps evolving exactly, only the readout is
                # sampled. Columns past the local 3n block are nan -- a mixed correlator
                # such as sigma_x^i sigma_y^j would need a fourth setting, so the
                # three-setting budget does not cover them, and a consumer that reads one
                # must fail rather than silently mix exact and sampled features.
                measurement_results[i] = np.nan
                measurement_results[i, :3 * self.n] = shots.sample_local_pauli(
                    shots.basis_probabilities(rho, self._basis_rotations),
                    self._outcome_signs, self.n_shots, self._shot_rng,
                )
            else:
